...
def main():
    ...
    chargerTypeInfo = json.loads(sys.argv[8])  # 충전타입
    ...
    if int(input_cnt) >= 3:
        ...
        popup = folium.Popup(iframe, max_width=350)
        folium.Marker(location=[latitude, longitude], icon=folium.Icon(icon='star',color='pink'), tooltip = input_name, popup = popup).add_to(map)
    ...
    for i in availableCount:
        ...
        if int(i['안내']) >= 5:
          # iframe = folium.IFrame('<b>충전소명</b> : ' + str(i['충전소명']) + '<br><br>' + '<b>주소</b> : ' + str(i['주소']) + '<br><br>'+ '<b>충전가능 대수</b> : ' + str(i['안내'])+'대'+ '<br><br>' +'<b>전화번호</b> : ' +str(i['전화번호']) + '<br><br>' +'<b>사용시간</b> : ' +str(i['사용시간']) + '<br><br>'+'<b>주차료</b> : '+ isFree, height = 260)
            iframe = folium.IFrame(template, height =220)
